    ...
    return (
      <div className="max-w-4xl mx-auto px-6 text-center py-20">
        <div className="glass-strong p-16">
          <h2 className="text-2xl font-bold text-white mb-3">{t('loginRequired')}</h2>
          <Link to="/login" className="btn-premium mt-4 inline-block">{tc('shopNow')}</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-5xl mx-auto px-6 pb-16">
      ...
      {loading ? (
        ...
        <div className="glass-strong text-center p-16 animate-fade-in-up">
          ...
          <p className="text-gray-300 mb-6">{t('emptySubtitle')}</p>
          ...
          {orders.map((order, i) => (
            ...
            >
              ...
              <div className="flex flex-wrap items-center justify-between gap-4 mb-5">
                <div>
                  <h3 className="text-lg font-bold text-white">{t('orderNumber', { id: order.id })}</h3>
              ...
              {order.shipping_address && (
                <div className="mb-5 p-3 rounded-xl bg-white/5 border border-white/10">
                  <p className="text-sm text-gray-200 font-medium">
                    📍 <span className="font-semibold text-white">{t('shippingAddress')}:</span> {order.shipping_address}
                  </p>
                </div>
              )}
                ...
                {order.items.map((item) => (
                  <div key={item.id} className="flex items-center gap-3 p-3 rounded-xl bg-white/8 border border-white/10">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {item.size && <p className="text-xs text-gray-400 mt-0.5 font-medium">{t('size')}: {item.size}</p>}
              ...
              <div className="mt-4 pt-4 border-t border-white/10 flex items-center justify-between">
                ...
                <p className="text-sm font-bold text-white">
                  {t('total')}: <span className="text-purple-300">${order.total.toFixed(2)}</span>
                </p>
              </div>
            </div>
          ))}
